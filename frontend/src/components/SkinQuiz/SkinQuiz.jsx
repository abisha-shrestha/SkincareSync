...
//         setResult(null);
//     };
//     const fetchRoutine = async (skinType) => {
//         setRoutineLoading(true);
//         try {
//             const res = await fetch(`http://localhost:3000/api/products/routine?skinType=${skinType}`);
//             const data = await res.json();
//             if (data.success) setRoutine(data.routine.filter(Boolean));
//         } catch (err) {
//             console.error(err);
//         } finally {
...
export default function SkinQuiz() {
    ...
    useEffect(() => {
        const checkExisting = async () => {
            if (userEmail) {
                ...
                const session = sessionStorage.getItem('skinType');
                if (session) {
                    setExistingSkinType(session);
                    fetchRoutine(session);
                }
            }
            setLoadingExisting(false);
        };
    ...
    const RoutineSection = ({ skinType }) => (
        (routine.length > 0 || routineLoading) && (
            <div className="quiz-routine">
                <p className="quiz-routine-label">Your 3-Step Routine</p>
                {routineLoading ? (
                    <p style={{ color: 'var(--text-muted)', fontSize: '14px', textAlign: 'center' }}>
                        Building your routine...
                    </p>
                ) : (
                    <div className="quiz-routine-steps">
                        {routine.map((product, i) => (
                            <div
                                key={product._id}
                                className="quiz-routine-card"
                                onClick={() => navigate(`/product/${product._id}`)}
                            >
                                <div className="quiz-routine-step-badge">Step {i + 1}</div>
                                <div className="quiz-routine-img">
                                    {product.imageUrl && (
                                        <img src={product.imageUrl} alt={product.name} />
                                    )}
                                </div>
                                <div className="quiz-routine-info">
                                    <span className="quiz-routine-category">{product.category}</span>
                                    <p className="quiz-routine-name">{product.name}</p>
                                    <p className="quiz-routine-price">Rs. {product.price.toLocaleString()}</p>
                                </div>
                            </div>
                        ))}
                    </div>
        ...
        <div className="quiz-overlay">
            ...
            {existingSkinType && !quizMode && !result && (
                <div className="quiz-container quiz-result-container">
                    ...
                    <RoutineSection skinType={existingSkinType} />
                    <div className="quiz-result-actions">
                        <button className="quiz-retake-btn" onClick={handleStartQuiz}>
                            Retake Quiz
                        </button>
                        <button className="quiz-shop-btn" onClick={() => navigate('/products', { state: { skinType: existingSkinType } })}>
                            Shop for my skin type
                        </button>
                    </div>
                </div>
            ...
            {result && (
                <div className="quiz-container quiz-result-container">
                    ...
                    <RoutineSection skinType={result} />
                    <div className="quiz-result-actions">
                        <button className="quiz-retake-btn" onClick={handleRetake}>
                            Retake Quiz
                        </button>
                        <button className="quiz-shop-btn" onClick={() => navigate('/products', { state: { skinType: result } })}>
                            Shop for my skin type
                        </button>
                    </div>
                </div>
